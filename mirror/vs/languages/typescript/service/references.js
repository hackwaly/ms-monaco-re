...
define(["require", "exports", '../lib/typescriptServices', 'vs/base/objects', 'vs/base/lib/winjs.base'], function(
  ...
  var Graph = (function() {
    ...
    Graph.prototype.removeNode = function(name) {
      ...
      // delete references to this node
    ...
    Graph.prototype.toJSON = function() {
      ...
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i],
          outgoing = this.store[key].getOutgoing();

        graph.push(i);
        graph.push(outgoing.length);
        for (var j = 0; j < outgoing.length; j++) {
          var pathOutgoing = outgoing[j],
            indexOutgoing = index[pathOutgoing];
  ...
  /**
   * Returns the transitive closure of the graph so that
   * typescript likes it.
   * @param graph A dependency graph
   */
  function computeTransitiveClosure(graph) {
    // insert from the graph
    var root, nodes = [],
      name, seen = {};
  ...
  /**
   * Builds a fully resolved dependency graph that contains the provided files.
   */
  function buildDependencyGraph(loader, files, options) {
    var graph = new Graph(),
      collector = new ScannerBasedCollector(),
      file, alreadyInGraph;

    // we will shift from this array
    files = files.slice(0);

    // heuristic: start with files with many references
    // so that we maybe include files with less references
    files.sort(function(a, b) {
      return b.references.length - a.references.length;
    });

    return new winjs.Promise(function(c, e, p) {
      // resolve the first file, build the graph from it
      // and recurse until we have seen all files
      function f() {
        if (files.length === 0) {
          c(graph);
          return;
  ...
  var File = (function() {
    ...
    File.prototype.resolve = function(loader, collect, options, seen) {
      ...
      return new winjs.Promise(function(success, error, progress) {
        ...
        if (c === 0) {
          // No references, we are done
          success(null);
          return;
        }

        _this.references.forEach(function(reference) {
          // Load each reference and store success or failure
    ...
    Scanner.prototype.next = function() {
      if (this.nextTokens.length === 0) {
        var token = this.scanner.scan([], true),
          ...
          currentOffset = this.scanner.absoluteIndex();
    ...
    ScannerBasedCollector.prototype.consume = function(value) {
      ...
      while (nextToken().kind !== typescriptServices.TypeScript.SyntaxKind.EndOfFileToken) {
        if (token.kind === typescriptServices.TypeScript.SyntaxKind.ImportKeyword) {
          ...
          // for instance /// <reference path="boo" />
          var comment = token.text,
            offset = token.offset,
            length = token.length,
            match = TripleSlashReference.REGEXP.exec(comment);
