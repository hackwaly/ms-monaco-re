  ...
  var q = function() {
    ...
    a.prototype._onEditorMouseMove = function(a) {
      ...
      if (b === m.MouseTargetType.OVERLAY_WIDGET && a.target.detail === p.ModesGlyphHoverWidget.ID && !a.event[c])
        return;
      b === m.MouseTargetType.CONTENT_TEXT ? (this._glyphWidget.hide(), this._contentWidget.startShowingAt(a.target.range)) :
        b === m.MouseTargetType.GUTTER_GLYPH_MARGIN ? (this._contentWidget.hide(), this._glyphWidget.startShowingAt(a
          .target.position.lineNumber)) : this._hideWidgets();
    };
