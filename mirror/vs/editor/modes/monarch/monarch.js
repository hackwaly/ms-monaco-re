define('vs/editor/modes/monarch/monarch', [
  'require',
  'exports',
  'vs/platform/platform',
  'vs/editor/modes/stream',
  ...
  var y = function(e) {
    function t(n, i, o, r) {
      e.call(this, n), this.id = t.ID++, this.lexer = i, this.stack = o ? o : [i.start], this.embeddedMode = r ? r :
        null, this.embeddedEntered = !1, this.groupActions = null, this.groupMatches = null, this.groupMatched = null,
        this.groupRule = null;
    }
    return __extends(t, e), t.prototype.makeClone = function() {
      ...
      if (this.groupActions)
        i = this.groupActions.length, r = this.groupMatches, a = this.groupMatched.shift(), u = this.groupActions.shift(),
          d = this.groupRule, 0 === this.groupActions.length && (this.groupActions = null, this.groupMatches = null,
            this.groupMatched = null, this.groupRule = null);
      else {
        if (e.eos())
          return {
            type: ''
      ...
      if ('string' == typeof u || u instanceof Array)
        ...
        if (v = u.token, u.tokenSubst && (v = s.substituteMatches(this.lexer, v, a, r, o)), u.nextEmbedded && ('@pop' ===
          ...
          '@' === y[0] && (y = y.substr(1)), s.findRules(this.lexer, y) ? this.stack[0] = y : s.throwError(this.lexer,
            'trying to switch to a state \'' + y + '\' that is undefined in rule: ' + d.name), c = null;
        } else if (u.transform && 'function' == typeof u.transform)
          this.stack = u.transform(this.stack), c = null;
        else if (u.next)
          if ('@push' === u.next)
          ...
          '@' === y[0] && (y = y.substr(1)), s.findRules(this.lexer, y) ? this.stack.unshift(y) : s.throwError(this.lexer,
      ...
      if (null === v && (s.throwError(this.lexer, 'lexer rule has no well-defined action in rule: ' + d.name), v =
        ...
        this.groupActions && this.groupActions.length > 0 && s.throwError(this.lexer, 'groups cannot be nested: ' + d
          .name), r.length !== v.length + 1 && s.throwError(this.lexer,
          'matched number of groups does not match the number of actions in rule: ' + d.name);
        for (var _ = 0, b = 1; b < r.length; b++)
          _ += r[b].length;
        return _ !== a.length && s.throwError(this.lexer,
          'with groups, all characters should be matched in consecutive groups in rule: ' + d.name), this.groupMatches =
          r, this.groupMatched = r.slice(1), this.groupActions = v.slice(0), this.groupRule = d, e.goBack(a.length),
          this.tokenize(e);
      }
      if ('@rematch' === v && (e.goBack(a.length), a = '', r = null, v = ''), 0 === a.length)
        if (n !== this.stack.length || o !== this.stack[0] || (this.groupActions ? this.groupActions.length : 0) !==
          ...
          s.throwError(this.lexer, 'no progress in tokenizer in rule: ' + d.name), e.advanceToEOS();
      if (0 === v.indexOf('@brackets')) {
        var C = v.substr('@brackets'.length),
          w = l(this.lexer, a);
        return w || (s.throwError(this.lexer, '@brackets token returned but no bracket defined as: ' + a), w = {
          token: '',
          bracketType: 0
        }), {
  ...
  var _ = function(e) {
    function t(t) {
      !t.name && t.lexer && (t = t.lexer), e.call(this, t.name, t.workerScriptPath ? t.workerScriptPath : s.monarchPath +
        'Worker', t.usesEmbedded), this.lexer = t, this.modesRegistry = n.Registry ? n.Registry.as(r.Extensions.EditorModes) :
        null;
    }
  ...
  t.MonarchMode = _;
  var b = {};
  t.createEditorMode = f, t.register = g, t.getLanguage = v;
})