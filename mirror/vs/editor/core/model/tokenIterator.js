define(["require", "exports"], function(a, b) {
  var c = function() {
    function a(a, b) {
      this._model = a, this._currentLineNumber = b.lineNumber, this._currentTokenIndex = 0, this._currentLineTokens =
        this._model.getInternalTokens(this._currentLineNumber), this._next = null, this._prev = null;
      var c = b.column - 1,
        d = Number.MAX_VALUE;
      for (var e = this._currentLineTokens.length - 1; e >= 0; e--) {
        if (this._currentLineTokens[e].startIndex <= c && c <= d) {
          this._currentTokenIndex = e, this._next = this._current(), this._prev = this._current();
          break
        }
        d = this._currentLineTokens[e].startIndex
      }
    }
    return a.prototype._advanceNext = function() {
      this._prev = this._next, this._next = null;
      if (this._currentTokenIndex + 1 < this._currentLineTokens.length) this._currentTokenIndex++, this._next = this._current();
      else {
        while (this._currentLineNumber + 1 <= this._model.getLineCount()) {
          this._currentLineNumber++, this._currentLineTokens = this._model.getInternalTokens(this._currentLineNumber);
          if (this._currentLineTokens.length > 0) {
            this._currentTokenIndex = 0, this._next = this._current();
            break
          }
        }
        this._next === null && (this._currentLineTokens = this._model.getInternalTokens(this._currentLineNumber),
      ...
      this._next = this._prev, this._prev = null;
      if (this._currentTokenIndex > 0) this._currentTokenIndex--, this._prev = this._current();
      else
