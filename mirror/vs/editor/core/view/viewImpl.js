define("vs/editor/core/view/viewImpl", ["require", "exports", "vs/nls!vs/editor/editor.main", "vs/base/dom/dom",
  "vs/base/eventEmitter", "vs/editor/core/constants", "vs/editor/core/view/viewContext",
  "vs/editor/core/controller/keyboardHandler", "vs/editor/core/controller/pointerHandler",
  "vs/editor/core/view/parts/zones/zones", "vs/editor/core/view/lines/viewLines",
  "vs/editor/core/view/parts/overviewRuler/overviewRuler",
  "vs/editor/core/view/parts/overviewRuler/decorationsOverviewRuler",
  "vs/editor/core/view/parts/viewCursors/viewCursors", "vs/editor/core/view/parts/contentWidgets/contentWidgets",
  "vs/editor/core/view/parts/overlayWidgets/overlayWidgets", "vs/editor/core/view/layout/layoutProvider",
  "vs/editor/core/view/viewEventHandler", "vs/editor/core/view/viewEventDispatcher", "vs/editor/core/range",
  "vs/base/env", "vs/base/errors", "vs/editor/core/view/viewController", "vs/editor/core/view/viewOverlays",
  "vs/editor/core/view/overlays/currentLineHighlight/currentLineHighlight",
  "vs/editor/core/view/overlays/selections/selections", "vs/editor/core/view/overlays/decorations/decorations",
  ...
  "vs/editor/core/view/overlays/scrollDecoration/scrollDecoration", "vs/base/lifecycle"
  ...
  var R = function(e) {
    function t(t, n, i) {
      ...
      this.outgoingEventBus = new o.EventEmitter;
      var l = new w.ViewController(i, n, this.outgoingEventBus);
      this.listenersToRemove = [];

      this.listenersToDispose = [];

      this.eventDispatcher = new y.ViewEventDispatcher(function(e) {
        return r._renderOnce(e);
      });

      this.linesContent = document.createElement("div");

      this.linesContent.className = s.ClassNames.LINES_CONTENT;

      this.domNode = document.createElement("div");

      this.domNode.className = n.getEditorClassName();

      this.overflowGuardContainer = document.createElement("div");

      this.overflowGuardContainer.className = s.ClassNames.OVERFLOW_GUARD;

      this.layoutProvider = new m.LayoutProvider(n, i, this.eventDispatcher, this.linesContent, this.domNode);

      this.eventDispatcher.addEventHandler(this.layoutProvider);

      this.context = new s.ViewContext(t, n, i, this.eventDispatcher, function(e) {
      ...
      this.keyboardHandler = new a.KeyboardHandler(this.context, l, this.createKeyboardHandlerHelper());

      this.pointerHandler = new u.PointerHandler(this.context, l, this.createPointerHandlerHelper());
      ...
      this.listenersToRemove.push(i.addBulkListener(function(e) {
        return r.eventDispatcher.emitMany(e);
      }));
    }
    __extends(t, e);

    t.prototype.createTextArea = function() {
      var e = this;
      this.textArea = document.createElement("textarea");

      this.textArea.className = s.ClassNames.TEXTAREA;
      ...
      this.textArea.setAttribute("aria-label", n.localize("vs_editor_core_view_viewImpl", 0));
      ...
      this.textArea.style.fontSize = this.context.configuration.editor.fontSize + "px";

      this.textArea.style.lineHeight = this.context.configuration.editor.lineHeight + "px";

      this.listenersToDispose.push(i.addDisposableListener(this.textArea, "focus", function() {
      ...
      this.textAreaCover.className = this.context.configuration.editor.glyphMargin ? s.ClassNames.GLYPH_MARGIN + " " +
        s.ClassNames.TEXTAREA_COVER : this.context.configuration.editor.lineNumbers ? s.ClassNames.LINE_NUMBERS + " " +
        s.ClassNames.TEXTAREA_COVER : "monaco-editor-background " + s.ClassNames.TEXTAREA_COVER;

      this.textAreaCover.style.position = "absolute";

      this.textAreaCover.style.width = "1px";

      this.textAreaCover.style.height = "1px";
    ...
    t.prototype.createViewParts = function() {
      ...
      this.viewLines = new c.ViewLines(this.context, this.layoutProvider);

      this.viewZones = new l.ViewZones(this.context, this.layoutProvider);
      ...
      this.viewParts.push(n);

      n.addDynamicOverlay(new T.GlyphMarginOverlay(this.context));

      n.addDynamicOverlay(new N.LinesDecorationsOverlay(this.context));

      n.addDynamicOverlay(new M.LineNumbersOverlay(this.context));

      n.addDynamicOverlay(new x.SelectionsOverlay(this.context));

      n.addDynamicOverlay(new L.DecorationsOverlay(this.context));

      n.addDynamicOverlay(new D.ScrollDecorationOverlay(this.context));

      n.addOverlay(new S.CurrentLineHighlightOverlay(this.context));

      this.contentWidgets = new f.ViewContentWidgets(this.context);
      ...
      this.linesContent.appendChild(i.getDomNode());

      this.overflowGuardContainer.appendChild(n.domNode);
    ...
    t.prototype.createKeyboardHandlerHelper = function() {
      ...
      return {
        ...
        visibleRangeForPositionRelativeToEditor: function(t, n) {
          ...
          var o = e.viewLines.visibleRangesForRange2(new _.Range(t, n, t, n), i.visibleRangesDeltaTop, !1);
          return o ? o[0] : null;
        }
      };
    };
    ...
    t.prototype.onLayoutChanged = function(e) {
      this.domNode.style.width = e.width + "px";

      this.domNode.style.height = e.height + "px";

      this.overflowGuardContainer.style.width = e.width + "px";

      this.overflowGuardContainer.style.height = e.height + "px";

      this.linesContent.style.width = e.contentWidth + "px";

      this.linesContent.style.height = e.contentHeight + "px";

      this.linesContentContainer.style.left = e.contentLeft + "px";

      this.linesContentContainer.style.width = e.contentWidth + "px";

      this.linesContentContainer.style.height = e.contentHeight + "px";

      this.outgoingEventBus.emit(r.EventType.ViewLayoutChanged, e);
    ...
    t.prototype.onViewFocusChanged = function(e) {
      i.toggleClass(this.domNode, "focused", e);

      e ? this.outgoingEventBus.emit(r.EventType.ViewFocusGained, {}) : this.outgoingEventBus.emit(r.EventType.ViewFocusLost, {});
    ...
    t.prototype.dispose = function() {
      ...
      this.listenersToDispose = I.disposeAll(this.listenersToDispose);
    ...
    t.prototype.focus = function() {
      ...
      i.selectTextInInputElement(this.textArea);
    ...
    t.prototype.change = function(e) {
      ...
      this._renderOnce(function() {
        ...
        n && t.context.privateViewEventBus.emit(r.EventType.ViewZonesChanged, null);
    ...
    t.prototype.addContentWidget = function(e) {
      var t = this;
      this._renderOnce(function() {
        t.contentWidgets.addWidget(e.widget);

        t.layoutContentWidget(e);
      });
    };
    ...
    t.prototype.addOverlayWidget = function(e) {
      var t = this;
      this._renderOnce(function() {
        t.overlayWidgets.addWidget(e.widget);

        t.layoutOverlayWidget(e);
      });
    };
    ...
    t.prototype._scheduleRender = function() {
      ...
      i.runAtThisOrScheduleAtNextAnimationFrame(function() {
        e._renderNow();
      });
    };
    ...
    t.prototype.createRenderingContext = function(e) {
      ...
      var n = this.layoutProvider.getCurrentViewport();
      ...
      var i = {
        ...
        getViewportVerticalOffsetForLineNumber: function(e) {
          ...
          var i = t.layoutProvider.getScrolledTopFromAbsoluteTop(n);
          return i;
        },
        ...
        visibleRangeForPosition: function(n) {
          var i = t.viewLines.visibleRangesForRange2(new _.Range(n.lineNumber, n.column, n.lineNumber, n.column), e.visibleRangesDeltaTop, !
            1);
          return i ? i[0] : null;
        },
