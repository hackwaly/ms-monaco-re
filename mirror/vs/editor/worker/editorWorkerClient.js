...
define(["require", "exports", "vs/base/worker/workerClient", "vs/editor/core/constants", "vs/editor/editor"], function(
  ...
  var i = function(a) {
    function b(b, c, d) {
      a.call(this, new f.DefaultWorkerFactory(c), "vs/editor/worker/editorWorkerServer");

      this.languageModeModuleId = b;

      this.friendlyModuleId = this.languageModeModuleId;
      var e = this.friendlyModuleId.lastIndexOf("/");
      if (e >= 0) {
        this.friendlyModuleId = this.friendlyModuleId.substr(e + 1);
      }
      var g = {
      ...
      this.onModuleLoaded = this.request("initialize", g);
      ...
      this.modelsValidateTimeout = {};

      this.modelsValidateRequest = {};
    }
    __extends(b, a);

    b.prototype._decodeMessageName = function(a) {
      var b = a.type;
      a.type === "rawRequest" && (b = a.payload.name);

      return this.friendlyModuleId + " - " + b;
    };
    ...
    b.prototype.modelDecorationsUpdate = function(a) {
      if (this.models.hasOwnProperty(a.modelId)) {
        ...
        var c = [];
        for (var d in a.decorations) {
          if (a.decorations.hasOwnProperty(d)) {
            c.push(a.decorations[d]);
          }
        }
    ...
    b.prototype.unbindModel = function(a) {
      this.modelsUnbind[a.id]();

      delete this.modelsUnbind[a.id];

      delete this.models[a.id];

      delete this.modelsDecorations[a.id];

      if (this.modelsValidateTimeout.hasOwnProperty(a.id)) {
        window.clearTimeout(this.modelsValidateTimeout[a.id]);
        delete this.modelsValidateTimeout[a.id];
      }

      if (this.modelsValidateRequest.hasOwnProperty(a.id)) {
        this.modelsValidateRequest[a.id].cancel();
        delete this.modelsValidateRequest[a.id];
      }

      this.request("modelDestroy", [a.getAssociatedResource()], 0);
    };
    ...
    b.prototype._setupValidation = function(a) {
      ...
      var d = a.getAssociatedResource();
      if (this.modelsValidateTimeout.hasOwnProperty(c)) {
        window.clearTimeout(this.modelsValidateTimeout[c]);
        delete this.modelsValidateTimeout[c];
      }

      if (this.modelsValidateRequest.hasOwnProperty(c)) {
        this.modelsValidateRequest[c].cancel();
        delete this.modelsValidateRequest[c];
      }

      this.modelsValidateTimeout[c] = window.setTimeout(function() {
        delete b.modelsValidateTimeout[c];

        b.modelsValidateRequest[c] = b.request("validate", [d, !1]);
      }, 500);
    };
    ...
    b.prototype.bindModel = function(a) {
      ...
      this._setupValidation(a);

      this.models[a.id] = a;

      this.modelsUnbind[a.id] = a.addBulkListener(function(c) {
        var d = [];
        ...
        if (d.length > 0) {
          b._sendModelEvent(a, d);
        }

        if (e) {
          b._setupValidation(a);
        }
      });
    };
    ...
    b.prototype._onError = function(a, b) {
      console.error("[" + this.languageModeModuleId + "]: " + a);
